...
from pytorch_transformers import BertModel,BertForMaskedLM
import pdb
...
class BEncoder(nn.Module):
    '''
        Text vision joint encoder
    '''

    def __init__(self, vocab_size, embed_size, hidden_size, padding_idx,
            max_instr_len, layer_fn, num_layers, dropout, bert=None,update=False):
        ...
        self.update = update
        #self.positional_encoding = PositionalEncoding(embed_size, max_instr_len)
        self.positional_encoding = PositionalEncoding(hidden_size, max_instr_len)
        self.dropout = nn.Dropout(p=dropout)


        self.zero_layer = nn.Linear(embed_size, hidden_size)
        first_layer  = layer_fn(hidden_size, hidden_size)
        hidden_layer = layer_fn(hidden_size, hidden_size)
        self.layers = nn.ModuleList(
            [first_layer] + clone(hidden_layer, num_layers - 1))

    def forward(self, input, seq_mask, img_feats, *args):

        att_mask = ~seq_mask
        if self.bert.__class__.__name__ == 'DicModel':
            outputs = self.bert(input, token_type_ids=None,attention_mask=att_mask,img_feats=img_feats)
        else:
            outputs = self.bert(input, token_type_ids=None,attention_mask=att_mask)

        input = outputs[0]
        if not self.update:
            input = input.detach()
        input = self.zero_layer(input)
        input = input * math.sqrt(input.size(-1))
        output = self.dropout(self.positional_encoding(input))
...
class Seq2SeqModel(nn.Module):
    ...
    def __init__(self, vocab_size, input_size, state_size, hparams, device):
        ...
        if hparams.encoder_type == 'dic':
            bert = None
            if hparams.pretrain_model_name is not None and hparams.pretrain_model_name != "" :
                print("Using the pretrained lm model from %s" %(hparams.pretrain_model_name))
                premodel = DicAddActionPreTrain.from_pretrained(hparams.pretrain_model_name)
                bert = premodel.bert

            self.encoder = BEncoder(
                vocab_size,
                hparams.d_ctx_size,
                hidden_size,
                hparams.instr_padding_idx,
        ...
        elif hparams.encoder_type == 'purelang':
            bert = None
            if hparams.pretrain_model_name is not None and hparams.pretrain_model_name != "" :
                print("Using the pretrained lm model from %s" %(hparams.pretrain_model_name))
                premodel = BertForMaskedLM.from_pretrained(hparams.pretrain_model_name)
                bert = premodel.bert
        ...
        elif hparams.encoder_type == 'bert':
    ...
    def encode(self, seq, seq_mask,img_feats = None):
        if self.hparams.encoder_type in ['dic','purelang','bert']:
            return self.encoder(seq,seq_mask,img_feats=img_feats)
        else:
            return self.encoder(seq, seq_mask)


class BSeq2SeqModel(nn.Module):

    def __init__(self, vocab_size, input_size, state_size, hparams, device):

        super(BSeq2SeqModel, self).__init__()
    ...
    def encode(self, seq, seq_mask,img_feats = None):
        if self.hparams.encoder_type == 'dic':
            return self.encoder(seq,seq_mask,img_feats=img_feats)
        else:
            return self.encoder(seq, seq_mask)
...
class BNavModule(BSeq2SeqModel):
    ...
    def __init__(self, vocab_size, agent_class, hparams, device):
        ...
        super(BNavModule, self).__init__(
            vocab_size, nav_input_size, nav_state_size, hparams, device)
...
class AgentModel(nn.Module):
    ...
    def __init__(self, vocab_size, hparams, device):
        ...
        self.hparams = hparams
        if hparams.encoder_type in ['dic','purelang','bert']:
            #self.nav_module = BNavModule(vocab_size, agent_class, hparams, device)
    ...
    #def encode(self, ctx_seq, ctx_mask,img_feats=None):
    def encode(self, *args, **kwargs):
        #if self.hparams.encoder_type == 'dic':
        #    lst = list(args)
        #    return self.nav_module.encode(lst[0],lst[1],lst[2]), \
        #           self.ask_module.encode(lst[0],lst[1])
        #else:
