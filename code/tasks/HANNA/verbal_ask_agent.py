...
from utils import collect_action_embeds
import pdb
...
class VerbalAskAgent(AskAgent):
    ...
    def rollout(self):
        ...
        for time_step in range(self.episode_len):
            ...
            # Action Visual features
            all_view_features = collect_action_embeds(obs)
            ...
            if should_encode_instruction:
                ...
                if self.hparams.encoder_type in ['dic', 'purelang','bert']:
                    #if not (goal_view_features==0).all():
                    #    img_feat = torch.cat([curr_view_features,goal_view_features], dim=1)
                    #else:
                    #    img_feat = curr_view_features
                    img_feat = all_view_features.to(ctx_seq.device)
                    #nav_ctx, ask_ctx = self.model.encode(ctx_seq, ctx_mask,img_feat)
